...
cv2.imwrite('GaussianBlurImage.png', img_gb)
cv2.imwrite('MedianBlurImage.png', img_mb)
cv2.imwrite('BilateralFilteredImage.png', img_bf)
cv2.imwrite('Edges.png', img_lp_al)
cv2.imwrite('InvertedImage.png', inverted_original)
cv2.imwrite('ReducedImage.png', img_bins)
cv2.imwrite('CartoonImage.png', cartoon_Bilateral)


# Display the input and Cartooned images
plt.subplot(1, 2, 1)
...
plt.title('MedianBlurImage')

plt.subplot(1, 2, 2)
plt.imshow(cv2.cvtColor(img_bf, cv2.COLOR_BGR2RGB))
plt.title('BilateralFiltered Image')
...
plt.subplot(1, 2, 2)
plt.imshow(cv2.cvtColor(cartoon_Bilateral, cv2.COLOR_BGR2RGB))
